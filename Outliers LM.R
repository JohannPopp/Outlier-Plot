...
dat2 <- data.frame(outID = rownames(mod2$model), mod2$model, pred = predict(mod2), zResiduals = rstandard(mod2), dffit = dffits(mod2), dfbeta = dfbeta(mod2), covratio(mod2), cooks.distance(mod2), hatvalues(mod2))
...
#  selector <- input_select(colnames(dat2), selected = "pred", label = "X Value")
  ...
  # Plot 1
  scatPlot <- function(x, y = zResiduals, plotNo = "plot1") {
    dat2 %>% 
      ggvis(~x, ~y, key := ~outID) %>% 
  ...
  scatPlot(x = dat2$pred, y = dat2$zResiduals)
  ...
  # Plot 2
  scatPlot(dat2$pred, dat2$hatvalues.mod2., plotNo = "plot2")
  
  # Plot 3
  scatPlot(dat2$pred, dat2$cooks.distance.mod2., plotNo = "plot3")
